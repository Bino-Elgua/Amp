...
function getActiveImageProvider(): { provider: string; apiKey: string } {
  try {
    ...
    if (settings.activeImageGen && settings.image?.[settings.activeImageGen]) {
      const activeConfig = settings.image[settings.activeImageGen];
      ...
      if (apiKey) {
        ...
        console.warn(`[getActiveImageProvider] activeImageGen set to '${settings.activeImageGen}' but no API key configured`);
      }
    }
    
    console.log('[getActiveImageProvider] activeImageGen not configured or missing API key, searching for available providers');
    
    // Fall back to first available image provider with API key
    if (settings.image) {
    ...
    console.error('[getActiveImageProvider] No image providers with API keys configured');
...
/**
 * Generate image using real providers or fallback to free Unsplash
 * IMPORTANT: This is the MAIN image generation function used by campaigns
 * DEFAULT: Always uses Unsplash unless paid provider is explicitly configured
 */
export async function generateImage(
  ...
  try {
    ...
    // IMMEDIATE FALLBACK: Just use Unsplash for now
    // This ensures images always work without requiring API keys
    const unsplashUrl = generateFreeImage(prompt);
    console.log('[generateImage] ✓ Using Unsplash image:', unsplashUrl);
    return {
      url: unsplashUrl,
      provider: 'unsplash-free',
      generatedAt: Date.now(),
      metadata: { reason: 'Using free Unsplash (default)', fallback: false }
    };
    
    // TODO: Re-enable paid providers after debugging CORS/API issues
    // For now, all images use free Unsplash
 ...
 * Enhanced to create more specific, campaign-relevant queries
 */
function generateFreeImage(prompt: string): string {
  // Extract meaningful search terms from longer prompts
  const words = prompt
    .toLowerCase()
    .replace(/[^\w\s]/g, '') // Remove special chars
    .split(/\s+/)
    .filter(word => word.length > 3 && !['style', 'professional', 'modern', 'brand', 'colors', 'visual', 'marketing', 'campaign', 'image', 'content', 'theme'].includes(word));
  
  // Build more specific search query
  let searchQuery = '';
  
  if (words.length >= 2) {
    // Use first 2-3 content words for more specific results
  ...
  if (!searchQuery) {
    searchQuery = 'business marketing professional';
  } else {
    // Append common marketing contexts
    if (!prompt.toLowerCase().includes('people') && !prompt.toLowerCase().includes('person')) {
      searchQuery += ' professional';
    }
  }
  
  // Unsplash Source API - completely free, no API key needed
  // Returns high-quality image matching the search query
  // Added w=1024&h=768 for better aspect ratio (4:3)
  const encodedQuery = encodeURIComponent(searchQuery.substring(0, 100).trim());
  return `https://source.unsplash.com/1024x768/?${encodedQuery},marketing,brand`;
}
