...
import { apiValidator, ApiKeyValidationResult } from './apiProviderValidator';
...
class ValidationService {
    ...
    /**
     * Validate API Key format for any provider (delegates to apiProviderValidator)
     * Now supports 70+ providers across LLM, Image, Voice, Video, Workflow, Email
     */
    validateApiKey(provider: string, apiKey: string): { valid: boolean; message: string; category?: string } {
        const result = apiValidator.validateApiKey(provider, apiKey);
        return {
            valid: result.valid,
    ...
    /**
     * Test API key validity by making a test request (delegates to apiProviderValidator)
     */
    async testApiKey(provider: string, apiKey: string): Promise<{ valid: boolean; message: string }> {
        const result = await apiValidator.testApiKey(provider, apiKey);
        return {
            valid: result.valid,
            message: result.message,
        };
    ...
    /**
     * Get list of all supported providers by category
     */
    getSupportedProviders(category?: 'llm' | 'image' | 'voice' | 'video' | 'workflow' | 'email'): Record<string, any> {
        if (category) {
            const providers = apiValidator.getProvidersByCategory(category);
            return providers.reduce((acc, p) => {
                acc[p.name.toLowerCase()] = p;
                return acc;
            }, {} as Record<string, any>);
        }
        return apiValidator.listAllProviders();
    }

    /**
     * Get validation stats
     */
    getProviderStats(): { totalProviders: number; byCategory: Record<string, number> } {
        return apiValidator.getStats();
    }
}
