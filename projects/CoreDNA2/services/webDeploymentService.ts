...
class WebDeploymentService {
  ...
  async deployToFirebase(website: WebsiteContent, projectName: string): Promise<DeploymentResult> {
    ...
    try {
      // Firebase Realtime Database REST API for hosting files
      const projectId = projectName.toLowerCase().replace(/\s+/g, '-');
      const timestamp = Date.now();
      
      // Create a deployment via Firebase REST API
      const deployResponse = await fetch(
      ...
      if (!deployResponse.ok) {
        // Fallback: Firebase requires CLI for direct hosting deploys
        // Provide clear instructions for Firebase deployment
        return {
          success: false,
          error: 'Firebase Hosting requires Firebase CLI for direct deployments. Use Vercel or Netlify for cloud deployment.',
          provider: 'firebase',
        };
      }

      const site = await deployResponse.json() as any;
      const url = site.defaultUrl || `https://${projectId}.web.app`;
      ...
      return {
        success: false,
        error: e.message || 'Firebase deployment failed. Use Vercel or Netlify instead.',
        provider: 'firebase',
      };
    }
  }
