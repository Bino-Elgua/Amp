...
export const generateCampaignAssets = async (
  ...
  const targetAudience = dna.personas?.map((p: any) => p.name).join(', ') || dna.targetAudience || 'target audience';
  const painPoints = dna.personas?.flatMap((p: any) => p.painPoints)?.slice(0, 3).join(', ') || 'customer needs';
  const keyStrengths = dna.swot?.strengths?.slice(0, 2).join(', ') || 'brand strengths';
  
  const prompt = `You are a professional marketing strategist specializing in high-converting campaign copy. Generate ${count} in-depth campaign assets for "${dna.name}" across these channels: ${channelsList}.

BRAND CONTEXT:
Name: ${dna.name}
Tagline: ${dna.tagline}
Mission: ${dna.mission || dna.description}
Core Values: ${dna.values?.slice(0, 3).join(', ') || 'integrity, quality, innovation'}
Key Strengths: ${keyStrengths}
Target Personas: ${targetAudience}
Customer Pain Points: ${painPoints}
Brand Voice: ${dna.toneOfVoice?.description || 'professional and customer-focused'}
Key Messages: ${dna.keyMessaging?.slice(0, 3).join('; ') || 'delivering value'}
Colors: ${dna.colors?.map((c: any) => c.name || c.hex).join(', ') || 'brand palette'}
Visual Style: ${dna.visualStyle?.description || 'modern professional'}

CAMPAIGN GOAL: ${goal}

For EACH of the ${count} assets, create a comprehensive JSON object with:
- id: unique identifier (e.g., "asset_1")
- channel: ${channels.map(c => `"${c}"`).join('|')} (assign based on channel list)
- type: "image" | "carousel" | "email" | "blog" | "social"
- title: compelling, benefit-driven headline that sparks curiosity (8-12 words)
- content: 200-300 character body copy that:
  * Addresses a specific pain point from the target audience
  * Clearly explains how ${dna.name} solves it
  * Uses specific, concrete language (not generic)
  * Includes brand voice and tone
  * Ends with a compelling reason to take action
- cta: specific action button text (e.g., "Claim Free Consultation", not just "Learn More")
- hashtags: 3-5 relevant hashtags for the channel
- imagePrompt: ultra-detailed visual description (150+ words) including:
  * Specific scene composition and framing
  * Brand colors from: ${dna.colors?.map((c: any) => c.name).slice(0, 2).join(', ')}
  * Mood and visual style: ${dna.visualStyle?.description}
  * What emotion/action should the viewer feel
  * Include human elements if appropriate for the persona
  
CRITICAL REQUIREMENTS:
- Each asset must be UNIQUE and not generic
- Copy must reference specific benefits, not just the product name
- All assets must feel cohesive with the brand voice: "${dna.toneOfVoice?.adjectives?.slice(0, 3).join(', ')}"
- Content length: minimum 200 characters, not short/shallow
- Each title should have different angle (different pain point, benefit, or audience segment)

Return ONLY a valid JSON array of ${count} complete objects. No markdown, no text outside JSON.`;
  ...
  try {
    ...
    // ENSURE each asset has deep, meaningful content - critical for usability
    assets = assets.map((asset: any, idx: number) => {
      ...
      // Get content (priority: content > copy > body > text)
      let content = asset.content || asset.copy || asset.body || asset.text || '';
      if (typeof content !== 'string') {
        content = JSON.stringify(content);
      }
      content = content.trim();
      
      // Log content depth for debugging
      console.log(`[generateCampaignAssets] Asset ${idx + 1} content length: ${content.length} chars`);
      
      // Warn if content is too shallow (less than 150 chars)
      if (content.length < 150) {
        console.warn(`[generateCampaignAssets] ⚠️ Asset ${idx + 1} content is shallow (${content.length} chars). Expected 200+ chars.`);
      }
      
      // If no imagePrompt, generate one from the asset content
      let finalImagePrompt = (imagePrompt || '').trim();
      if (!finalImagePrompt || finalImagePrompt.length < 10) {
        ...
        // Use more content in the image prompt for better generation
        const contentSnippet = content.substring(0, 150);
        finalImagePrompt = `Professional marketing image for: "${titleText}". Content theme: "${contentSnippet}...". Visual style: ${visualStyle}. Brand colors: ${colors}. High-quality, realistic, suitable for ${asset.channel || 'social media'}.`;
      }
      ...
      return {
        ...
        content: content, // Use the deep content, not shallow
