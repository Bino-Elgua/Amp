...
const SchedulerPage: React.FC = () => {
    ...
    const { campaigns: globalCampaigns, loadCampaigns } = useCampaigns();
    ...
    useEffect(() => {
        loadLocalData();
        // Also sync global campaigns to queue
        console.log('[SchedulerPage] Global campaigns available:', globalCampaigns.length);
        // Load pending queue from localStorage (added by CampaignsPage)
        const storedQueue = localStorage.getItem('core_dna_pending_queue');
        if (storedQueue) {
            try {
                setQueue(JSON.parse(storedQueue));
            } catch(e) {
                console.error('[SchedulerPage] Failed to load queue:', e);
            }
        }
    ...
    // Reload queue when global campaigns change
    useEffect(() => {
        const storedQueue = localStorage.getItem('core_dna_pending_queue');
        if (storedQueue) {
            try {
