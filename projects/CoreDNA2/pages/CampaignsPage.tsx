...
const CampaignsPage: React.FC = () => {
  ...
  const { campaigns: globalCampaigns, saveCampaign, loadCampaigns } = useCampaigns();
  ...
  const [usedProviders, setUsedProviders] = useState<{ llm: string; image: string }>({ llm: 'Unknown', image: 'Unknown' });
  ...
  // Load campaigns on mount from global context
  useEffect(() => {
    console.log('[CampaignsPage] Campaigns available from global context:', globalCampaigns.length);
  }, [globalCampaigns]);
  ...
  const handleGenerate = async () => {
    ...
    try {
        ...
        setUsedProviders({
          llm: settings.activeLLM || 'Unknown',
          image: settings.activeImageGen || 'Unknown'
        });
                ...
                if (Array.isArray(generatedAssets)) {
                    ...
                    const imagePromises = generatedAssets.map(async (asset) => {
                        if (!asset || !asset.imagePrompt) {
                            console.warn('[CampaignsPage] Asset missing imagePrompt, generating from title:', asset?.title);
                            // Generate prompt from asset title if missing
                            const fallbackPrompt = `Marketing image for "${asset?.title || 'campaign'}". Style: professional`;
                            const fallbackResult = await generateImage(fallbackPrompt, { style: selectedDNA.visualStyle?.description || 'modern style' });
                            return { ...asset, imageUrl: fallbackResult.url, imagePrompt: fallbackPrompt, isGeneratingImage: false };
                        }
                        ...
                        try {
                            ...
                            console.error('[CampaignsPage] Generating fallback Unsplash image...');
                            // Still generate a fallback image
                            const fallbackResult = await generateImage(asset.imagePrompt || `Campaign: ${asset.title}`, { style: selectedDNA.visualStyle?.description || 'modern style' });
                            return { ...asset, imageUrl: fallbackResult.url, isGeneratingImage: false };
                        }
                    });
                    ...
                    if (assetsWithImages.length > 0) {
                        ...
                        await saveCampaign(newSavedCampaign);
                        console.log('[CampaignsPage] ✓ Campaign auto-saved to global context:', newSavedCampaign.id);
                        ...
                        if (enabledWorkflows.length > 0) {
                            ...
                            try {
                                // Trigger workflow for each enabled provider
                                ...
                                toastService.success('⚡ Campaign sent to workflow automation');
                                console.log('[CampaignsPage] ✓ Workflow automation triggered successfully');
                            } catch (wfErr: any) {
                                console.warn('[CampaignsPage] Workflow trigger failed:', wfErr);
                                // Don't show error toast - workflows are optional
                                console.log('[CampaignsPage] Workflows optional - continuing without automation');
                            }
                        }
                    }
                }
  ...
  return (
    ...
    <div className="container mx-auto px-4 py-8 pb-20">
        ...
        {assets.length > 0 && !loading && (
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-10">
                <div className="p-6 bg-green-900/20 border border-green-500 rounded-xl">
                    ...
                    <p className="text-green-200 text-xs mt-1">{usedProviders.llm} LLM + {usedProviders.image} Images</p>
                </div>
        ...
        <SavedCampaignsModal 
          ...
          savedCampaigns={globalCampaigns} 
          onLoad={(c) => { setAssets(c.assets); setGoal(c.goal); setIsSavedModalOpen(false); }} 
          onDelete={(id) => {
            const updated = globalCampaigns.filter(c => c.id !== id);
            localStorage.setItem('core_dna_saved_campaigns', JSON.stringify(updated));
            loadCampaigns(); // Reload from global context
          }}
        />
